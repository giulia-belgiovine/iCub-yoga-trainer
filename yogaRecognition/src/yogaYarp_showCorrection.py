            ...
            if input_yarp_image and self.user_name is not None:
                ...
                try:
                    ...
                    if self.train_phase:
                        ...
                        msg_instruction, show_correction = self.yoga_trainer.get_pose_feedback(current_pose)   # added by Mati
                        print(show_correction)  # added by Mati

                        if self.i % self.frequency_feedback == 0:
                            self.write_iSpeak(msg_instruction) # && self.show_yoga_pose(show_correction)    # added by Mati
                        self.i += 1
                        ...
                        if msg_evaluation:
                            self.write_iSpeak(msg_evaluation) # && self.show_yoga_pose(show_correction)    # added by Mati
                            self.process = False
